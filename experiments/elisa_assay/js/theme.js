 ...
 *
 * Carried over unchanged from the DNA Microarray bench except for the two
 * lines in sci() that name this experiment's colour tables.
 *
...
(function (Lab) {
   ...
   * @param {string} name  a SIGNAL key ('tmb'|'stopped'|'blank')
   *                       or a REAGENTS key ('DIL'|'AG'|'AB1'|'AB2'|'TMB'|…)
   * @param {string} [variant] 'base'|'deep' for SIGNAL; 'fill'|'dark' for reagents
   */
  function sci(name, variant) {
    var k = name + ':' + (variant || '');
    if (k in sciCache) return sciCache[k];
    var cfg = Lab.config;                       // resolved lazily — see header
    if (!cfg || !cfg.SIGNAL) return '#ff00ff';
    var src;
    if (cfg.SIGNAL[name])        src = cfg.SIGNAL[name][variant || 'base'];
  ...
  /* An ACHROMATIC source must stay achromatic.  A blank well is very nearly
     colourless — hue 0, saturation ~0 — so applying the minimum-saturation
     floor would drag it toward red and tint every negative well pink. */
