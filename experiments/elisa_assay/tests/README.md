...
## Latest free-hand correction

See [FREE_HAND_FIX.md](FREE_HAND_FIX.md) for the reproduced short-press regression and the 16 focused unit tests run for its correction. Free-hand `rise` now measures thumb-joint bending relative to the thumb's own base axis, not radial wrist distance. The calibrated real-pipette features and shared gesture gates are unchanged. The following sections record the earlier refinement and its validation, not a new webcam verification.

## September 2026 gesture refinement (earlier implementation)

The primary calibrated button features use normalized 3D distances between the wrist and thumb landmarks (0 through 4), rather than an across-knuckle coordinate frame. Free-hand plunger travel initially used the same wrist/thumb geometry; that free-hand measurement is superseded by the correction above. A usable palm frame is still used for free-hand ejector direction; its absence does not disable the plunger. Grip acquisition, cursor positioning, and hand rendering still use other hand landmarks.
...
Repeat in Free-hand gestures mode, allowing the raised thumb to settle briefly after pickup and fully releasing it between presses. Bend the thumb down while keeping the fingers closed. A held-down thumb must not repeat an action. Hide the hand briefly, bring it back with the thumb still down, then release: interrupted gestures must not replay. Check open-hand release and mouse ownership as well.
