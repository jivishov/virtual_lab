...
class Contact{
 ...
 update(f){let x=f.rawX??f.x,y=f.palmY??f.y;if(![x,y,f.now].every(Number.isFinite)||f.now<this.last||(this.source==='hand'&&f.now-this.last>300)){this.valid=false;this.reason='tracking';return this}this.last=f.now;let dx=x-this.x,dy=y-this.y,hand=this.source==='hand';if(Math.abs(dx)>(hand?.10:54)||dy<(hand?-.028:-12)||dy>(hand?.25:170)){this.valid=false;this.reason='withdrawal';return this}this.depth=clamp(dy/(hand?.042:28));if(this.depth<.18)this.rearmed=true;return this}
}
class Interaction{
